...
- Onboard SD NAND (CSNP1GCR01-BOW, 128MB) for log storage via SPI (SD protocol, FAT filesystem)
- 2 I2C buses
- 2-way analog camera switch
- RGB status LED (PE2/PE3/PE4) and WS2812 on PWM11
- Beeper
...
- `BATT2_CURR_PIN` = 9 (PB0)
- `BATT2_VOLT_MULT` = 11.0

The onboard current sensor values above apply to battery 1. For the external current sensor on PB0, set `BATT2_AMP_PERVLT` to match your sensor.
...
An analog RSSI input is available on PC5. `RSSI_ANA_PIN` defaults to 8.

## Camera Switch

GPIO 81 (PD11, PINIO1) controls the analog camera switch (CAM1/CAM2). `RELAY1_PIN` defaults to 81.
...
The PWM output 11 (PA8) is dedicated to WS2812 LED control. Board and NeoPixel notification is enabled by default. To use it:
...
| PA1   | 57   | PWM8 / MOTOR8   |
| PA2   | 58   | PWM9 / MOTOR9   |
| PA3   | 59   | PWM10 / MOTOR10 |
| PA8   | 60   | PWM11 / NeoPixel|
| PA15  | 32   | Beeper          |
| PD11  | 81   | Camera switch   |
| PD3   | 70   | CAN silent      |

## I2C Buses

- I2C1: PB6 (SCL), PB7 (SDA) - external compass/GPS
- I2C2: PB10 (SCL), PB11 (SDA) - onboard barometer (SPL06/DPS310)
...
| SD NAND  | SPI3 | PD7     | Onboard SD NAND (CSNP1GCR01-BOW), SD spec 2.0 SPI |
...
The board includes an onboard SD NAND chip (CSNP1GCR01-BOW, 1Gbit / 128MB) connected
via SPI3 (CS: PD7). The chip implements SD Specification 2.0 and is accessed in SPI
mode using ArduPilot's MMC-SPI driver. A FAT filesystem is mounted at `/` for data
logging.

Flight logs are written to `/APM/LOGxxxx.BIN` on the onboard storage. If the storage
fails to mount at boot, try increasing `BRD_SD_SLOWDOWN`.
